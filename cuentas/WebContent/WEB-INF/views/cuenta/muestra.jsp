...
<body>
	...
	<form action="modificarCuenta" method="post">
		...
		<br /> Valor: <br /> <input type="text" name="valor"
			value="${cuenta.valor}" /> <br /> Tipo: <br /> <select name="tipo">
			<option value="INGRESO" ${cuenta.tipo=='INGRESO' ? 'selected':''}>Ingreso</option>
			<option value="SALIDA" ${cuenta.tipo=='SALIDA' ?
'selected':''}>Salida</option>
		</select> <br /> ¿Pago? <input type="checkbox" name="pago"
			${cuenta.pago?'checked':''} /> <br /> Fecha de Pago: <input
			type="text" name="fechaPago"
			value="<fmt:formatDate
value="${cuenta.fechaPago.time}"
pattern="dd/MM/yyyy" />" />
		<br /> <input type="hidden" name="id" value="${cuenta.id}" /> <input
